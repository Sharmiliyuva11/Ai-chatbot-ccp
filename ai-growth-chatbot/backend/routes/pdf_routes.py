...
def get_pdf_questions():
    ...
    if result.get('success') and result.get('reply'):
        ...
        try:
            ...
            ChatModel.save_message(user_id, f"Generate questions from {os.path.basename(pdf_path)}", result.get('reply'))
        except Exception:
            pass
...
def explain():
    ...
    if result.get('success') and result.get('reply'):
        try:
            user_id = get_jwt_identity()
            ChatModel.save_message(user_id, f"Explain {os.path.basename(pdf_path)}", result.get('reply'))
        except Exception:
            pass
...
def answer_pdf():
    ...
    if result.get('success') and result.get('reply'):
        try:
            user_id = get_jwt_identity()
            ChatModel.save_message(user_id, user_query, result.get('reply'))
        except Exception:
            pass
...
# =========================
# 📌 Generate 16-Mark Questions from PDF
# =========================
@pdf_routes.route('/generate-16-mark', methods=['POST'])
@jwt_required()
def generate_16_mark_question():
    data = request.get_json() or {}
    pdf_path = data.get("path")
    filename = data.get("filename")  # Allow filename instead of full path
    
    # If filename provided, construct path
    if filename and not pdf_path:
    ...
    # Generate 16-mark question
    result = answer_from_pdf(text, "give me 16 mark question")
    if result.get('success') and result.get('reply'):
        try:
            user_id = get_jwt_identity()
            ChatModel.save_message(user_id, f"Generate {16}-mark Q from {os.path.basename(pdf_path)}", result.get('reply'))
        except Exception:
            pass
        return jsonify({"success": True, "question": result['reply'], "filename": os.path.basename(pdf_path)})
    else:
        return jsonify({"success": False, "message": result.get('error', 'Failed to generate 16-mark question')})
